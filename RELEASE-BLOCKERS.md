...
- [ ] **Trademark and vendor sanction.** The extension is named "Idira Integration Importer" and keeps the vendor term "Idira" in that name by deliberate choice, not oversight. It also automates the vendor's admin UI without vendor sanction. This is an open item flagged for counsel to review, not an engineering one: public distribution under a mark that includes a third-party vendor's term is still trademark use regardless of channel, and an unsanctioned integration could draw objection from the vendor. Distributing via GitHub rather than the Chrome Web Store (see "Distribution method undecided" below) reduces exposure somewhat — there is no store listing to pull and no store reviewer to flag it — but it does not eliminate the question. Strategic risk alongside it: if the vendor ships a native "Install" action, this extension becomes redundant.
